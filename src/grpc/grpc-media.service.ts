...
interface MediaServiceGrpc {
  ...
  getPrivateMediaUrls(data: { files: string[] }): Promise<any>;
}
...
export class GrpcMediaService {
  ...
  onModuleInit() {
    this.mediaService =
      this.client.getService<MediaServiceGrpc>("MediaService");
  }

  async getSignedUploadUrls(files: string[]): Promise<any> {
    return lastValueFrom(
      await this.mediaService.getSignedUploadUrls({ files })
    );
  }
  ...
  async getPrivateMediaUrls(files: string[]): Promise<any> {
    return lastValueFrom(
      await this.mediaService.getPrivateMediaUrls({ files })
    );
  }
}
